...
def part_one():
    ...
    for i in prod_data:
        ...
        while j <= word_size:
            ...
            j = j + 1
        while word_size >= 0:
            if i[word_size].isnumeric():
                last_digit = i[word_size]
                # print("L" + last_digit)
                break
            word_size = word_size - 1
        two_digit_number = first_digit + last_digit
        # print("T" + two_digit_number)
        sum_of_numbers = sum_of_numbers + int(two_digit_number)
    print("S = " + str(sum_of_numbers))
...
def part_two():
    ...
    sum_of_numbers = 0
    for e in prod_data:
        first_position, first_value, last_position, last_value = check_for_number(e, numbers)
        j = 0
        first_digit = ""
        last_digit = ""
        word_size = len(e) - 1
        while j <= word_size:
            ...
            j = j + 1
        while word_size >= 0:
            if e[word_size].isnumeric():
                last_digit = e[word_size]
                # print("L" + last_digit)
                break
            word_size = word_size - 1
        # if first_digit is not empty
        if first_digit:
            if first_position is not None:
                if first_position < e.find(first_digit):
                    first_digit = str(first_value)
        else:
            first_digit = str(first_value)

        # if last_digit is not empty
        if last_digit:
            if last_position is not None:
                if last_position > e.rfind(last_digit):
                    last_digit = str(last_value)
        else:
            last_digit = str(last_value)
        """
        if first_position < int(first_digit):
            first_digit = str(first_value)
        if last_position > int(last_digit):
            last_digit = str(last_value)
        """
        two_digit_number = first_digit + last_digit
        print("T" + two_digit_number)
...
def check_for_number(x: str, y: dict):
    """
    :param x: String which will be checked
    :param y: Dictionary of substrings and their values which is checked for
    :return: position and value of found substring for first and last numeric string
    """
    first_position = None
    first_value = -1
    last_position = None
    last_value = -1
    # check for first numeric string
    for e in y:
        f = x.find(e)
        if f != -1:
            if first_position is None:
                first_position = f
                first_value = y[e]
            elif f < first_position:
                first_position = f
                first_value = y[e]
    # check for last numeric string
    for e in y:
        f = x.rfind(e)
        if f != -1:
            if last_position is None:
                last_position = f
                last_value = y[e]
            elif f > last_position:
                last_position = f
                last_value = y[e]

    return first_position, first_value, last_position, last_value


test = ["tsjvxqljbfivefive11fourfour1st"]
